...
export default function newsletterForm() {
    ...
    const inputStyle = `mb-4 py-4 rounded-lg pl-6 `

    return(
        <div>
            <form onSubmit={handleSubmit} className="flex flex-col mt-6 mb-5">
                <div className=" font-RobotoBold mb-1 flex justify-between">
                ...
                <input 
                    ...
                    required
                    className={`${inputStyle} ${email === '' ? 'border-gray-400' : isSubmitClicked && !isValid ? 'border-red-400 bg-red-200 text-red-400' : 'border-gray-400'}`}
                />
                <button className="bg-[hsl(234,29%,20%)] text-white py-4 rounded-lg">Subscribe to monthly newsletter</button>
            </form>
        </div>
    )
}
